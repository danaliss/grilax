...
public class EventController {
	...
	/**
	 * Gets a list of all the events for the currently logged in user. Roles:
	 * Anonymous
	 * 
	...
	/**
	 * Create cookout Roles: Host
	 * 
	 * @param event    Event object to create
	 * @param response HTTP response object
	 * @return new event object <strong>HTTP 201</strong> Created <strong>HTTP
	 *         400</strong> Bad request
	 */
	...
	/**
	 * Views the cookout details for the specified event. Roles: Attendee | Host |
	 * Chef
	 */
	...
	/**
	 * Gets the attendees from the specified event. Roles: Attendees | Host
	 */
	...
	/**
	 * Adds an attendee to the specified event. Roles: Host
	 */
	@PostMapping(path = "/event/{eventid}/attendees")
	public Response addEventAttendee(EventAttendees attendee, @PathVariable long eventid,
			HttpServletResponse response) {
		response.setStatus(HttpServletResponse.SC_CREATED);
		return new Response(eventDao.addEventAttendee(attendee));
	}
...
//    }
	...
	/**
	 * Updates the specified event. Roles: Host
	 */
